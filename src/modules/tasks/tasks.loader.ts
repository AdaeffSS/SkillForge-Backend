...
export class TaskLoaderService {
  ...
  constructor() {
    console.log('TaskLoaderService instance created');
  }
  ...
  async importAllTasks(): Promise<any[]> {
    ...
    for (const taskClass of tasksClasses) {
      ...
      try {
        ...
        this.logger.error(`Data loading error for task ${exam}.${subject}.${taskKey}`, err);
        throw err;
      }
    }

    this.logger.log('Tasks classes import and templates/parameters loading are complete');
    return tasksClasses;
  }
  ...
  private async importTaskClasses(): Promise<any[]> {
    ...
    for (const filePath of entries) {
      try {
        ...
        this.logger.log(`The file is successfully imported: ${relative(`${rootPath}/dist/modules/tasks/`, filePath)}`);
      } catch (error) {
        this.logger.error(`Error when importing file: ${relative(`${rootPath}/dist/modules/tasks/`, filePath)}`);
      }
    }
    return tasksClasses;
  }
  ...
  private async loadParameters(): Promise<void> {
    ...
    for (const filePath of files) {
      try {
        ...
        const relativePath = filePath.slice(basePath.length + 1);
        const key = relativePath.replace(/\/parameters\.yaml$/, '').toLowerCase();
  ...
  getTemplate(exam: string, subject: string, taskKey: string): string {
    ...
    const taskFolder = lastUnderscore !== -1 ? taskKey.substring(0, lastUnderscore) : taskKey;
    ...
    if (!template) {
      this.logger.setContext(TaskLoaderService.name)
      console.log("!", this.templates)
      this.logger.error(`Template not found for task ${taskKey} in ${exam}/${subject} for key: ${key}`);
      throw new Error(`Template not found for task ${taskKey} in ${exam}/${subject} for key: ${key}`);
    }
    return template;
  }

  getParameters(exam: string, subject: string, taskKey: string): Record<string, any> {
    const lastUnderscore = taskKey.lastIndexOf('_');
    const taskFolder = lastUnderscore !== -1 ? taskKey.substring(0, lastUnderscore) : taskKey;
    ...
    if (!params) {
      this.logger.setContext(TaskLoaderService.name)
      this.logger.error(`Parameters not found for task ${taskKey} in ${exam}/${subject} for key: ${key}`);
      throw new Error(`Parameters not found for task ${taskKey} in ${exam}/${subject} for key: ${key}`);
    }
    return params;
  }
}
